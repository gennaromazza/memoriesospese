...
In this workspace, `protobufjs@7.5.4` is a direct root dependency whose metadata is available but whose tarball is denied by the Socket Security Policy (Critical CVE). The firewall’s HTTP endpoint is reachable, while npm’s plain lockfile resolution can rewrite the URL to HTTPS and report `ECONNREFUSED`. npm 11 may surface the failure only as `Exit handler never called!`; inspect the preceding fetch lines before changing application code. Without changing the lockfile or obtaining a firewall exception, no compatible local cache may exist.