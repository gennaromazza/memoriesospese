...
4. **Invia allo studio per verifica** congela la modifica cliente. Lo studio vede **Da verificare**, il riepilogo e i contatti dei clienti del lavoro; **Apri WhatsApp** apre la conversazione, senza inviare messaggi automaticamente.
5. Lo studio può correggere la configurazione; una correzione di una proposta in verifica/confermata non riabilita implicitamente il cliente. **Richiedi modifiche al cliente** restituisce la proposta con un messaggio. Per il cliente restano validi i blocchi legacy dell’impaginato/in stampa; lo studio può comunque intervenire sulla versione corrente senza riaprire quei blocchi.
...
- Ogni salvataggio ricontrolla versione, associazione lavoro/galleria, stato, revisione e proposta in transazione. L’admin può modificare il mockup corrente senza modificare il lock dell’impaginato. Una sessione obsoleta riceve 409: il pulsante **Ricarica proposta** consente di recuperare lo stato aggiornato chiedendo conferma prima di scartare modifiche locali.
