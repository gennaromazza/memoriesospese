...
- **Email System:** **Email notifications moved to Express.js server** (October 28, 2025) due to Firebase Cloud Functions network restrictions preventing access to Replit Connectors API. Gmail API integration via Replit OAuth2 connector with server-side authentication (Firebase ID token verification), authorization (gallery ownership/admin check), and recipient resolution from Firestore subscriptions collection. Token caching and robust error handling. Endpoint: `/api/email/notify-new-photos` (authenticated, authorized). Firebase Cloud Functions still handle password requests via separate secure endpoints.
