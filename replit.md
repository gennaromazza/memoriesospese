...
- **Booking System:** Campaign-based photography booking with Google Calendar integration for slot management. Atomic booking flow prevents double-booking using Firestore transactions. Two-stage email notification (received and confirmed). Booking duration automatically calculated. **Slider Advance Feature:** Campaigns support `giorniAnticipoSlider` field (0-30 days) allowing homepage slider to appear X days before bookings actually open. Homepage displays slider when `oggi >= (dataInizio - giorniAnticipoSlider)`. **Booking Flexibility Control:** New `bloccaPrenotazioniPrimaInizio` boolean flag allows admins to independently control when bookings open - if false (default), bookings are always open even before `dataInizio`; if true, booking page shows informative banner and hides form until actual `dataInizio`, preventing premature bookings. Admin panel includes validated input (0-30 clamping for slider days) with clear UX messaging and toggle for booking restrictions. **Boolean Persistence:** Uses nullish coalescing (`??`) instead of logical OR (`||`) in create/update mutations to properly persist explicit false values in Firestore, ensuring toggles work correctly end-to-end. **Smart Navigation:** Cross-tab navigation with automatic scroll and highlight - clicking "Booking" button from Gestione Commesse navigates to bookings tab, resets filters, calculates correct pagination, scrolls to target card with smooth animation, and applies temporary visual highlight (3-second blue ring). System waits for data loading and handles edge cases (booking not found, pagination, filters). **Past Slot Protection:** Public booking page automatically disables time slots that have already passed by comparing slot start time with current time using `isBefore()`, preventing invalid bookings and displaying "Non disponibile" label on disabled slots.
...
- **Special Theme System:** Modular seasonal gallery system with predefined themes (Natale, Carnevale, San Valentino, Pasqua, Halloween) using dedicated CSS files with custom animations and elements. Galleries can be assigned a theme with mandatory PIN-based access. **PIN Uniqueness System:** Server-side validation via `/api/email/check-pin-unique` endpoint prevents duplicate PINs across special galleries using Firebase Admin SDK. Client-side validation displays toast error before save. **Automatic PIN Notification:** When PIN is created or modified, automatic email sent to client via `/api/email/special-gallery-pin-notification` endpoint with themed HTML template, PIN code, access link, and instructions. Client email/name fields persisted in gallery document for automatic notifications. Email sending includes 500ms delay to ensure Firestore propagation, detailed logging for debugging, and robust error handling with informative toast messages. Absolute URLs generated client-side using `createAbsoluteUrl()` to avoid environment variable dependencies.
- **Order & Gallery Management Integration:** Integrated order and gallery creation within BookingsManager admin panel. Supports multi-product orders, auto-calculation, and dynamic badging for created orders. Auto-generates gallery codes and pre-populates fields based on booking data. **Custom Products:** Admins can add custom products directly during order creation/modification with optional catalog persistence via toggle. Products marked "one-time" have empty prodottoId and display "Custom" badge. **Gallery Custom Product Support:** NewGalleryModal and EditGalleryModal fully support custom products - NewGalleryModal accepts `prePopulate` with prodottoNome/prodottoNumeroFoto for custom products, validates numeroFoto with safe fallback, and displays "Custom" (purple) or "Catalogo" (blue) badge. EditGalleryModal loads associated product from order/booking and displays product info with badge. OrdersManager displays "Custom" badge in both main order cards and detail dialogs when order contains custom products. **Multi-Product Gallery Creation:** When creating a gallery from a booking with an associated multi-product order, NewGalleryModal displays a dropdown selector allowing admin to choose which product to use. Dropdown shows product name, photo count, and "Custom" indicator. State management ensures clean reset when switching between bookings (selectedProductIndex resets via bookingId dependency) and handles edge cases (zero-photo products, fetch failures) by explicitly cleaning selection state. BookingsManager automatically passes all order products via `availableProducts` array, falling back to single booking product for legacy compatibility. **Order Modification:** Complete EditOrderModal allows updating products, prices, status, notes with automatic total recalculation. Email notification automatically sent to client upon save via `/api/orders/:id` PATCH endpoint with responsive HTML template showing updated order details.
