...
## Recent Changes
- **2025-01-15:** Interactive Workflow Timeline - WorkflowTimeline component enhanced with actionable buttons: "Appuntamento Visione File" (generates pre-filled consultation links via email/WhatsApp) and "Richiedi Appuntamento" (opens calendar modal with pre-selected client). Backend endpoints `/api/jobs/:id/send-consultation-request` and `/api/jobs/:id/timeline-events` save dynamic events to `job.workflowEvents[]` array. Timeline now displays recent activities (consulenze inviate, appuntamenti creati) in real-time.
- **2025-01-15:** Unified Quote Portal - Single link `/quote/:token` with adaptive rendering based on quote status (pre-signature vs signed), eliminating client confusion from multiple URLs.
- **2025-01-15:** Instagram-ready admin email notification on quote signature featuring creative gradient design, emoji, and social-media-optimized content. Auto-triggered via `/api/quotes/admin-quote-signed-notification` endpoint.
- **2025-01-15:** Integrated QuoteManagementPanel into JobDetailPage with expandable quote list, Firestore Timestamp normalization, cache-aligned React Query keys, and client-side sorting (avoiding composite index requirement).
...
- **Quote Management System:** Automated email notifications for quote lifecycle (sent, accepted, payment reminders), robust backend protection for signed quotes, and admin-only reset signature action. **Unified Client Portal:** Single public link `/quote/:token` auto-adapts to quote status - renders signature page pre-signing, payment schedule + contract post-signing (eliminates multiple confusing URLs). **Instagram-Ready Admin Notifications:** Creative, social-media-optimized email sent to admin when client signs (gradient design, emoji, shareable content via `/api/quotes/admin-quote-signed-notification`). **Advanced Manual Status Management:** Admin workflow for manual quote/contract status override with Firebase-authenticated endpoints (`GET /api/quotes/:id/status/validate`, `PATCH /api/quotes/:id/status`, `PATCH /api/quotes/:id/signature/manual`), preflight validation with financial safeguards (blocks status changes when payments are recorded), token revocation tracking via `revokedTokens[]` array, comprehensive audit logging in `quoteAuditLog` collection, and manual signature insertion for retroactive/forced signing. QuoteManagementPanel UI component provides status dropdown, confirmation dialogs with warnings, manual signature modal, audit log viewer, and revoked token display - **now integrated into JobDetailPage** with expandable quote list. System prevents data inconsistencies by centralizing business logic in `validateQuoteStatusChange()`. Security: Endpoints protected by `verifyAdminAuth` middleware with Firebase ID token verification (email-based admin check), eliminating x-admin-email header spoofing vulnerabilities.
