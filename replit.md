...
- **Transaction & Payment Tracking System:** Advanced payment tracking with `transactions` array, automated email notifications for payments, and dedicated registration buttons. **Event-Relative Payment Schedules:** Payment due dates can be set relative to event date (e.g., "-30" = 30 days before event, "+7" = 7 days after). GestioneRataModal automatically infers relative offsets when editing existing payments and preserves mode/offset intelligently. Falls back to absolute dates when eventDate is unavailable with clear UX feedback.
