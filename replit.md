...
- **Email System:** **ALL email operations moved to Express.js server** (October 28, 2025) due to Firebase Cloud Functions network restrictions preventing access to Replit Connectors API. Gmail API integration via Replit OAuth2 connector with:
  - **New Photo Notifications** (`/api/email/notify-new-photos`): Server-side authentication (Firebase ID token), authorization (gallery ownership/admin), recipient resolution from Firestore subscriptions. Fully authenticated and authorized.
  ...
  - **Security**: CORS restricted to authorized domains, comprehensive input validation, structured error handling without sensitive data leakage.
