...
- **Email System:** **ALL email operations moved to Express.js server** (October 28, 2025) due to Firebase Cloud Functions network restrictions preventing access to Replit Connectors API. Gmail API integration via Replit OAuth2 connector with:
  - **New Photo Notifications** (`/api/email/notify-new-photos`): Enterprise-grade security with Firebase ID token authentication via middleware, server-side owner/admin authorization, and recipient resolution from Firestore subscriptions. Subscription creation uses `serverTimestamp()` for Firestore Security Rules compatibility (October 29, 2025). Authentication middleware uses Firebase API key from environment variables (`process.env.VITE_FIREBASE_API_KEY`) for proper token validation. SubscriptionPrompt UI component integrated in Gallery.tsx with grid-aware full-width layout to avoid photo overlap in masonry grid.
