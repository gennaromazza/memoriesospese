...
- **Deployment:** Designed for deployment in a subfolder (e.g., `/wedgallery/`) with dynamic base path detection (`VITE_BASE_PATH`). Production server uses Express.js to serve static files and handle API routes, with a strong emphasis on Firebase-only architecture for core functionalities.
