...
- **Photobook Review System (Fotolibri):** Client-facing photobook proof review via token link (`/fotolibro/:token`). Clients draw colored X marks on pages to request replace/delete/edit changes (drafts sent in batch, server-side snapshot validation via `snapshotUrlPrefix`); clients can delete already-sent requests (any status) with best-effort snapshot regeneration for remaining marks. Admin can lock a photobook ("Manda in Stampa" in PhotobooksManager): `Photobook.locked` enforced server-side (403 on snapshot/submit/delete by-token), client sees read-only "Album mandato in stampa" banner; only admin can unlock. Replacement photo picker supports large galleries (search, chapter filter from `galleries/{id}.chapters[]` + `chapterId` on photos, progressive rendering in 60-photo batches). Admin page deletion cascade-deletes the change requests referencing that page; version `pageCount` updates run in Firestore transactions; client note capped at 2000 chars server-side. Routes: `server/photobook-routes.ts`; types: `shared/photobook-types.ts`. Il trasferimento pagine → Google Drive (POST /:id/lab-shipment) gira in background: la route risponde subito (202) e scrive avanzamento/esito in `labShipments/{id}.pageTransfer` (running/completed/partial/failed, heartbeat per pagina, guardia anti doppio avvio); il client polla GET /api/lab-shipments/:id e mostra barra di progresso e retry idempotente in PhotobooksManager.
