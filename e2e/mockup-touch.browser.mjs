...
vite=await createMockupHarnessServer(root,{define:{'import.meta.env.VITE_MOCKUP_RENDERER_READY_TIMEOUT_MS':JSON.stringify(String(RENDERER_TIMEOUT_MS))}});
 ...
 assert.equal(saved.selection.modelId,offer.options[0].id,'La bozza salvata appartiene ancora a Custodia');
  const revisionBeforeContextLoss=saved.revision;
  const draftSaveRequestsBeforeContextLoss=draftSaveRequests;
  await withTimeout('simulazione perdita WebGL',()=>frame.locator('canvas').evaluate(canvas=>canvas.dispatchEvent(new Event('webglcontextlost',{cancelable:true}))),TOUCH_TIMEOUT_MS);
  await withTimeout('errore perdita WebGL',()=>page.getByTestId('mockup-renderer-error').waitFor(),TOUCH_TIMEOUT_MS);
  assert.equal(await page.getByText('L’anteprima 3D si è interrotta. Riprova a caricarla.',{exact:true}).count(),1,'La perdita WebGL mostra una recovery localizzata');
  assert.equal(saved.revision,revisionBeforeContextLoss,'La perdita WebGL non crea una revisione');
  assert.equal(draftSaveRequests,draftSaveRequestsBeforeContextLoss,'La perdita WebGL non salva una nuova revisione');
  await touch(page.getByRole('button',{name:'Riprova a caricare l’anteprima',exact:true}),'retry dopo perdita WebGL');
  await waitForRenderer(frame,'renderer Custodia pronto dopo perdita WebGL');
  assert.equal(await frame.locator('#coverOptions select').inputValue(),'full','Il retry dopo perdita WebGL ripristina la copertina salvata');
  assert.equal(saved.revision,revisionBeforeContextLoss,'Il retry dopo perdita WebGL mantiene la revisione');
  assert.equal(draftSaveRequests,draftSaveRequestsBeforeContextLoss,'Il retry dopo perdita WebGL non salva una revisione');
  await withTimeout('simulazione fatal error post-ready',()=>frame.locator('body').evaluate(body=>body.ownerDocument.defaultView.dispatchEvent(new ErrorEvent('error',{message:'Errore renderer simulato'}))),TOUCH_TIMEOUT_MS);
  await withTimeout('recovery fatal error post-ready',()=>page.getByTestId('mockup-renderer-error').waitFor(),TOUCH_TIMEOUT_MS);
  assert.equal(await page.getByText('L’anteprima 3D si è interrotta. Riprova a caricarla.',{exact:true}).count(),1,'Il fatal error post-ready mostra una recovery localizzata');
  assert.equal(saved.revision,revisionBeforeContextLoss,'Il fatal error post-ready non crea una revisione');
  assert.equal(draftSaveRequests,draftSaveRequestsBeforeContextLoss,'Il fatal error post-ready non salva una nuova revisione');
  await touch(page.getByRole('button',{name:'Riprova a caricare l’anteprima',exact:true}),'retry dopo fatal error post-ready');
  await waitForRenderer(frame,'renderer Custodia pronto dopo fatal error');
  assert.equal(await frame.locator('#coverOptions select').inputValue(),'full','Il retry dopo fatal error ripristina la copertina salvata');
  assert.equal(saved.revision,revisionBeforeContextLoss,'Il retry dopo fatal error mantiene la revisione');
  assert.equal(draftSaveRequests,draftSaveRequestsBeforeContextLoss,'Il retry dopo fatal error non salva una revisione');
