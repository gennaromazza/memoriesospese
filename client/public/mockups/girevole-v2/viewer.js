...
let backPhoto = null, backPhotoMap = null, configurationRevision = 2;
function edited() { configurationRevision = 2; }
...
async function previews() {
  ...
  try {
    ...
    return previews;
  } finally { album.position.x = oldX; pivot.rotation.y = oldAngle; ground.visible = true; renderer.setPixelRatio(oldRatio); renderer.setSize(oldSize.x, oldSize.y, false); last = 0; renderer.setAnimationLoop(renderLoop); }
}
...
if (embedded) {
  ...
  window.addEventListener('message', async event => {
    ...
    try {
      ...
      if (c) {
        if (c.modelId !== modelId || ![1, 2].includes(c.assetRevision)) throw new Error('Revisione del modello non supportata');
