...
// Le incisioni a righe restano tali finché il cliente non inserisce i due nomi.
function edited() { configurationRevision = 4; updateBack(); }
...
for (const z of [-.04, .04]) {
  ...
  const glass = new THREE.Mesh(new THREE.ShapeGeometry(shape), plex); glass.position.z = z; glass.name = z < 0 ? 'Plexiglass posteriore scrigno' : 'Plexiglass anteriore scrigno'; pivot.add(glass);
...
// Stampa sul pannello posteriore già presente: resta nel pivot anche estraendo il libro.
// La sagoma e l'incavo coincidono con la lastra, non con le dimensioni della copertina.
const boxPhotoGeometry = pivot.getObjectByName('Plexiglass posteriore scrigno').geometry.clone();
const photoUV = boxPhotoGeometry.attributes.uv;
for (let i = 0; i < photoUV.count; i++) photoUV.setXY(i, (.216 - photoUV.getX(i)) / .432, (photoUV.getY(i) + .151) / .302);
const boxPhoto = new THREE.Mesh(boxPhotoGeometry, new THREE.MeshPhysicalMaterial({ map: backMap, roughness: .25, clearcoat: 1, clearcoatRoughness: .035, side: THREE.BackSide }));
boxPhoto.name = 'Stampa foto plexiglass posteriore scrigno'; boxPhoto.position.z = -.0402; pivot.add(boxPhoto);
...
function configuration() {
  ...
  if (monogramEnabled && configurationRevision >= 3) c.engravingNames = { first: $('firstName').value.trim(), second: $('secondName').value.trim() };
  return c;
}
function summary() {
...
function updateCover() {
  ...
  $('legacyEngraving').hidden = monogramEnabled;
  if (monogramEnabled) drawMonogram(inscriptionCanvas, $('firstName').value, $('secondName').value);
...
function updateBack() {
  ...
  rearPhoto.visible = rearPlex.visible = active && configurationRevision < 4;
  boxPhoto.visible = boxPhotoBacking.visible = active && configurationRevision >= 4;
  $('backPhotoControls').hidden = !active;
  ...
  if (backPhotoMap) {
    ...
    const areaAspect = configurationRevision >= 4 ? .432 / .302 : 4 / 3;
    const sw = image.width * Math.min(1, areaAspect / aspect) / zoom, sh = image.height * Math.min(1, aspect / areaAspect) / zoom;
...
function view(direction = [-.5, .32, 1]) {
  if (phoneView) {
    camera.position.copy(controls.target).add(new THREE.Vector3(...direction));
    controls.minDistance = .09;
    if (fitPhoneProduct(product, camera, controls)) return;
  }
...
for (const [id, factor] of [['plus', .8], ['minus', 1.25]]) $(id).onclick = () => {
  if (phoneView) controls.minDistance = phoneDetailDistance(product, camera, controls);
  const offset = camera.position.clone().sub(controls.target); offset.setLength(THREE.MathUtils.clamp(offset.length() * factor, controls.minDistance, controls.maxDistance)); camera.position.copy(controls.target).add(offset); controls.update();
};
if (phoneView) controls.addEventListener('change', () => { controls.minDistance = phoneDetailDistance(product, camera, controls); });
let homePose;
...
if (embedded) {
  ...
  $('backPhotoStatus').textContent = 'Scegli la foto per il plexiglass posteriore dello scrigno. La stampa non segue l’album estratto.';
  window.addEventListener('message', async event => {
    ...
    try {
      ...
      if (c) {
        if (c.modelId !== modelId || ![1, 2, 3, 4].includes(c.assetRevision)) throw new Error('Revisione del modello non supportata');
        configurationRevision = c.assetRevision;
        monogramEnabled = !!c.engravingNames;
