    ...
    <DialogContent ref={dialog} data-mobile-mockup={mobile ? 'true' : undefined} className="flex flex-col gap-0 p-0 sm:p-0 w-screen sm:w-[96vw] max-w-none sm:max-w-[1500px] h-[100dvh] sm:h-[94dvh] max-h-[100dvh] rounded-none sm:rounded-lg overflow-hidden [&>button]:hidden" onInteractOutside={event => event.preventDefault()}>
      {mobile && isPortrait && !picker && !choosing && <div className="mockup-rotate" role="alert" data-testid="mockup-rotate">
       ...
       {token && !mobile && <div className="mockup-progress shrink-0 border-b px-3 py-2" aria-live="polite">
         <div className="mockup-progress-summary"><p className="text-sm font-medium">Passaggio {activeStepIndex + 1} di {wizardSteps.length} · {activeStep.title}</p><span>{dirty ? 'Bozza da salvare' : saved ? MOCKUP_STATUS_LABELS[saved.status || 'draft'] : 'Bozza pronta'}</span></div>
         <div className="mockup-progress-steps" role="list" aria-label="Passaggi configuratore">{wizardSteps.map((item, index) => <span key={item.id} role="listitem" className={index < activeStepIndex ? 'is-done' : index === activeStepIndex ? 'is-current' : ''}><b>{index + 1}</b><em>{item.title}</em></span>)}</div>
       </div>}
