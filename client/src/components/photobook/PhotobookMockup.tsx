...
export default function PhotobookMockup({ photobookId, version, token, readOnly = false, summary = false, compact = false, onOpenChange }: Props) {
  ...
  if (token && !state.data?.enabled) {
    ...
    return compact
      ? <span className="max-w-36 text-xs leading-tight text-muted-foreground">Mockup in preparazione</span>
      : <section className="rounded-lg border border-blue-200 bg-blue-50 p-4 space-y-1" data-testid="photobook-mockup-pending" role="status"><h2 className="font-semibold text-blue-950">Personalizzazione in preparazione</h2><p className="text-sm text-blue-900">Le pagine sono approvate. Lo studio sta preparando la proposta per l’album: riceverai un aggiornamento quando potrai personalizzare copertina, rivestimento e scritte.</p></section>;
  }
  ...
  return <section className={compact ? 'inline-flex shrink-0 items-center gap-1.5' : 'rounded-lg border bg-white p-4 space-y-3'} data-testid="photobook-mockup">
    {compact ? <><Button className="h-10 shrink-0 bg-emerald-600 px-3 text-xs font-semibold text-white shadow-sm hover:bg-emerald-700" onClick={openConfigurator} aria-label={hasSavedDraft ? 'Recupera bozza album 3D' : saved ? 'Apri album 3D' : 'Crea album 3D'} data-testid="button-open-client-mockup">{saved?.status === 'changes_requested' ? 'Modifica album 3D' : hasSavedDraft ? 'Recupera album 3D' : saved ? 'Apri album 3D' : 'Crea album 3D'}</Button>{saved && <span className="max-w-28 truncate text-[11px] text-muted-foreground" title={clientStatusMessage}>{MOCKUP_STATUS_LABELS[saved.status || 'draft']}</span>}</> : <div className="flex flex-wrap items-center justify-between gap-2">
