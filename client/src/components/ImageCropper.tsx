...
export function ImageCropper({ image, onCropComplete, onCancel }: ImageCropperProps) {
  ...
  const drawCanvas = () => {
    ...
    // Disegna immagine con zoom e rotazione
    ctx.save();
    ...
    const cropWidth = (crop.width / 100) * 800;
    const cropHeight = (crop.height / 100) * 600;
  ...
  const handleOverlayClick = (e: React.MouseEvent) => {
    // Chiudi solo se si clicca sull'overlay, non sul contenuto
