...
export default function CreateJobModal({ open, onClose }: CreateJobModalProps) {
  ...
  const { items: provenances = [], isLoading: loadingProvenances } =
    useJobEntity("provenance");
    ...
    const checkConflicts = async () => {
      try {
        ...
        const response = await fetch(
          `/api/jobs/check-calendar?${params.toString()}`,
          {
  ...
  const handleRemoveCliente = (clienteId: string) => {
    ...
    form.setValue(
      "clientiIds",
      currentIds.filter((id) => id !== clienteId),
    );
  ...
  return (
    <>
      ...
      >
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Crea un nuovo lavoro fotografico. Potrai poi creare preventivi e
              gestire il workflow completo.
            </DialogDescription>
          </DialogHeader>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="job-type-select">
                        Tipo Lavoro *
                      </FormLabel>
                      <Select
                        value={field.value}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel htmlFor="provenance-select">
                        Provenienza *
                      </FormLabel>
                      <Select
                        value={field.value}
      ...
      <AlertDialog
        ...
        onOpenChange={(open) =>
          setConflictsAlert((prev) => ({ ...prev, open }))
        }
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Sono stati trovati {conflictsAlert.conflicts.length}{" "}
              {conflictsAlert.conflicts.length === 1 ? "evento" : "eventi"} che
              si sovrappongono con la data/orario selezionato:
            </AlertDialogDescription>
          </AlertDialogHeader>
