...
const manualSignatureSchema = z.object({
  ...
  signerName: z
    .string()
    .min(2, "Il nome del firmatario deve contenere almeno 2 caratteri"),
});
...
export default function ModuliJobSection({
  jobId,
  onCreateModulo,
  clienteId,
  isAdmin = false,
}: ModuliJobSectionProps) {
  const [selectedQuoteId, setSelectedQuoteId] = useState<string | null>(null);
  const [generaPagamentiQuoteId, setGeneraPagamentiQuoteId] = useState<
    string | null
  >(null);
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error: Error) => {
      ...
      if (
        error.message.includes("SIGNED_QUOTE_PROTECTION") ||
        error.message.includes("SIGNED_QUOTE_WITH_PAYMENTS")
      ) {
    ...
    return (
      <div className="text-center py-8">
        ...
        <p className="text-muted-foreground mb-4">
          Nessun modulo di prenotazione creato
        </p>
            ...
            onClick={() => {
              if (!jobId) {
                toast({
                  ...
                  description:
                    "Devi salvare il lavoro prima di creare un preventivo",
                  variant: "destructive",
                });
                return;
              }
  ...
  return (
    <div className="space-y-4">
                            ...
                            <>
                              <DropdownMenuItem
                                onClick={() => {
                                  ...
                                  if (
                                    selectedQuote.clientiInfo &&
                                    selectedQuote.clientiInfo.length > 0
                                  ) {
                                    const firstClient =
                                      selectedQuote.clientiInfo[0];
                                    manualSignatureForm.setValue(
                                      "signerName",
                                      `${firstClient.nome} ${firstClient.cognome}`,
                                    );
                                  }
                                }}
                ...
                <div className="space-y-6 mt-6">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant="outline">
                      {TYPE_LABELS[selectedQuote.type]}
                    </Badge>
                    ...
                    <div className="mt-4 pt-4 border-t flex justify-between items-center">
                      ...
                      <span className="font-bold text-2xl">
                        ...
                        {selectedQuote.type === "fisso"
                          ? (
                              selectedQuote.totaleBase ??
                              selectedQuote.totalAfterDiscount ??
                              0
                            ).toFixed(2)
                          : (
                              selectedQuote.totaleSelezionato ??
                              selectedQuote.totaleBase ??
                              selectedQuote.totalAfterDiscount ??
                              0
                            ).toFixed(2)}
                  ...
                  {selectedQuote.signature && (
                    <div className="border-t pt-4">
                      ...
                      <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-lg space-y-2">
                        <p className="text-sm">
                          <span className="font-medium">Firmato da:</span>{" "}
                          {selectedQuote.signature.clientName}
                        </p>
                        <p className="text-sm">
                          <span className="font-medium">Data:</span>{" "}
                          {formatDate(
                            selectedQuote.signature.signedAt.toDate(),
                            "dd/MM/yyyy HH:mm",
                            { locale: it },
                          )}
                        </p>
                        <p className="text-sm">
                          ...
                          {selectedQuote.signature.ipAddress}
                        </p>
                      </div>
                    </div>
                  )}
                  ...
                  {selectedQuote.status === "firmato" && isAdmin && (
                    <div className="border-t pt-4">
                      <Button
                        onClick={() =>
                          setGeneraPagamentiQuoteId(selectedQuote.id)
                        }
                        className="w-full"
                        size="lg"
                      ...
                      <p className="text-xs text-muted-foreground mt-2 text-center">
                        Crea uno scadenzario pagamenti basato sul totale
                        preventivato
                      </p>
                    </div>
                  )}
                  ...
                  <div className="border-t pt-4">
                    <h3 className="font-semibold mb-3">
                      {selectedQuote.status === "firmato"
                        ? "Portale Cliente Firmato"
                        : "Link Firma Preventivo"}
                    </h3>
                    <div className="flex items-center gap-2">
                      <input
                        type="text"
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            toast({
                              title: "Link copiato!",
                              description:
                                "Il link del portale è stato copiato negli appunti",
                            });
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          window.open(getQuoteUrl(selectedQuote), "_blank")
                        }
                    ...
                    {selectedQuote.status === "firmato" && (
                      <div className="mt-3">
                        ...
                        <p className="text-xs text-muted-foreground mt-2 text-center">
                          Apri il portale firmato per scaricare il PDF del
                          preventivo
                        </p>
                      </div>
                    )}
                ...
                <div className="bg-destructive/10 border border-destructive/30 rounded-lg p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Questo preventivo è stato firmato dal cliente.
                        Eliminarlo potrebbe:
                      </p>
      ...
      <Dialog
        open={!!resetQuoteId && signatureMode !== "none"}
        onOpenChange={(open) => {
          if (!open) {
            setResetQuoteId(null);
            setSignatureMode("none");
      ...
      >
        <DialogContent className="sm:max-w-md">
          ...
          {signatureMode === "reset" && (
            <div className="space-y-4">
              <div className="space-y-3 pt-2">
                <p className="text-sm text-muted-foreground">Questa azione:</p>
                <ul className="list-disc list-inside space-y-1 text-sm pl-2 text-muted-foreground">
                  <li>
                    Cambierà lo status da <strong>Firmato</strong> a{" "}
                    <strong>Bozza</strong>
                  </li>
                ...
                <div className="text-amber-700 bg-amber-50 dark:bg-amber-950/20 p-3 rounded-md border border-amber-200 dark:border-amber-800 text-sm">
                  ⚠️ Il cliente dovrà firmare nuovamente il preventivo se
                  necessario.
                </div>
              </div>
          ...
          {signatureMode === "manual" && (
            <form
              onSubmit={manualSignatureForm.handleSubmit((data) => {
                if (resetQuoteId) {
                  signatureMutation.mutate({
                    quoteId: resetQuoteId,
                    action: "manual",
              ...
              <div className="space-y-4 py-4">
                ...
                <div className="space-y-2">
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0" align="start">
                      <Calendar
                        ...
                        onSelect={(date) =>
                          date && manualSignatureForm.setValue("signedAt", date)
                        }
                  ...
                  {manualSignatureForm.formState.errors.signedAt && (
                    <p className="text-sm text-destructive">
                      {manualSignatureForm.formState.errors.signedAt.message}
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  {manualSignatureForm.formState.errors.signerName && (
                    <p className="text-sm text-destructive">
                      {manualSignatureForm.formState.errors.signerName.message}
                    </p>
                  )}
                </div>
                ...
                <div className="text-green-700 bg-green-50 dark:bg-green-950/20 p-3 rounded-md border border-green-200 dark:border-green-800 text-sm">
                  ℹ️ Questa firma verrà registrata come inserimento manuale per
                  import legacy.
                </div>
              </div>
