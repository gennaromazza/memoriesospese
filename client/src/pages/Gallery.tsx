              ...
              {activeTab === "photographer" && (
                <div>
                  ...
                  {displayPhotos.length === 0 ? (
                    ...
                    <div>
                      {/* ⚠️ Warning: Insufficient photos in gallery */}
                      {(() => {
                        // Calculate total required photos correctly for all modes
                        const totalRequiredForWarning = isMultiProductMode && productRequirements 
                          ? productRequirements.reduce((sum, p) => sum + (p.prodottoNumeroFoto || 0), 0)
                          : requiredPhotoCount;
                        
                        return isSelectionMode && selectionStatus !== "completed" && totalRequiredForWarning > 0 && photos.length < totalRequiredForWarning ? (
                          <div className="mb-6 bg-gradient-to-r from-yellow-100 to-orange-100 border-2 border-yellow-400 rounded-lg p-4 text-center">
                            <div className="flex items-center justify-center gap-2 text-yellow-700">
                              ...
                              <span className="font-semibold">
                                Attenzione: La galleria contiene solo {photos.length} foto, ma ne servono {totalRequiredForWarning} per la selezione.
                              </span>
                            </div>
