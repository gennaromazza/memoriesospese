 ...
 *
 * CREDENZIALI (obbligatorie, mai salvate in questo file):
 *   - FIREBASE_ADMIN_CREDENTIALS_FILE: percorso al JSON del service account; oppure
 *   - FIREBASE_ADMIN_CREDENTIALS: JSON puro o JSON codificato in base64; oppure
 *   - GOOGLE_APPLICATION_CREDENTIALS per Application Default Credentials.
 */
'use strict';
...
// ── CREDENZIALI (mai incorporate nel file) ────────────────────────────────────
const PROJECT_ID  = process.env.FIREBASE_PROJECT_ID || 'wedding-gallery-397b6';
const BUCKET      = PROJECT_ID + '.firebasestorage.app';
const GALLERY_URL = 'https://imagestudiofotografico.com/gallery';

function parseServiceAccount(rawValue) {
  const trimmed = String(rawValue || '').trim();
  if (!trimmed) throw new Error('Credenziali Firebase vuote.');
  ...
  if (!parsed.project_id || !parsed.client_email || !parsed.private_key) {
    throw new Error('Service account Firebase non valido.');
  }
  if (parsed.project_id !== PROJECT_ID) {
    throw new Error(`Il service account appartiene a ${parsed.project_id}, atteso ${PROJECT_ID}.`);
  }
  return parsed;
}
...
function loadFirebaseCredential() {
  if (process.env.FIREBASE_ADMIN_CREDENTIALS) {
    return admin.credential.cert(parseServiceAccount(process.env.FIREBASE_ADMIN_CREDENTIALS));
  }

  if (process.env.FIREBASE_ADMIN_CREDENTIALS_FILE) {
    const credentialPath = path.resolve(process.env.FIREBASE_ADMIN_CREDENTIALS_FILE);
    return admin.credential.cert(parseServiceAccount(fs.readFileSync(credentialPath, 'utf8')));
  }
