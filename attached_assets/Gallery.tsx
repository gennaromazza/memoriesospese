      ...
      try {
        ...
        if (galleryData.hasChapters) {
          try {
            ...
            console.error(
              "Errore nel caricamento dei capitoli:",
              chaptersError,
            );
          }
        }
        ...
        if (galleryData.hasChapters) {
          ...
          photosQuery = query(photosRef, limit(photosPerPage));
        }
        ...
        if (photosData.length === 0 && galleryData.photoCount > 0) {
          console.log(
            "Nessuna foto trovata nella sottocollezione, ma photoCount è",
          ...
          try {
            ...
            console.log(
              "Verifico percorso Storage:",
              `galleries/${galleryDoc.id}`,
            );
            ...
            if (listResult.items.length === 0) {
              ...
              try {
                ...
                if (spaceResult.items.length > 0) {
                  console.log(
                    "Trovate",
                    spaceResult.items.length,
                    "foto con path con spazio",
                  );
            ...
            if (
              ...
              galleryDoc.id === "kWraDKOW7MZiM5eHSM11"
            ) {
              console.log(
                "Provo con path hardcoded galleries/kWraDKOW7MZiM5eHSM11",
              );
              try {
                const hardCodedRef = ref(
                  storage,
                  `galleries/kWraDKOW7MZiM5eHSM11`,
                );
                const hardCodedResult = await listAll(hardCodedRef);
                if (hardCodedResult.items.length > 0) {
                  console.log(
                    "Trovate",
                    hardCodedResult.items.length,
                    "foto con path hardcoded",
                  );
            ...
            if (photosFromStorage.length > 0) {
              ...
              photosFromStorage.forEach(async (photo) => {
                try {
                  ...
                  console.error(
                    "Errore nel salvare i metadati in Firestore:",
                    err,
                  );
                }
              });
        ...
        toast({
          title: "Errore",
          description:
            "Si è verificato un errore nel caricamento della galleria.",
          variant: "destructive",
        });
      } finally {
        setIsLoading(false);
  ...
  return (
    <div className="min-h-screen bg-off-white">
      ...
      <div className="relative w-full overflow-hidden">
        {gallery.coverImageUrl ? (
          ...
          <div className="w-full h-64 md:h-80 bg-gradient-to-b from-sage/20 to-sage/5 flex items-center justify-center relative">
            <div className="md:w-72 w-60 h-auto">
              <DecorativeImage
                type="standing"
                alt={`Immagine coppia di sposi per ${gallery.name}`}
              />
            </div>
          </div>
        )}
        ...
        <div
          className={`absolute bottom-0 left-0 right-0 p-6 ${gallery.coverImageUrl ? "text-white" : ""}`}
        >
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <h1
              className={`text-3xl md:text-4xl font-bold leading-tight font-playfair ${gallery.coverImageUrl ? "drop-shadow-md text-white" : "text-blue-gray"}`}
            >
              {gallery.name}
            </h1>
            <p
              className={`mt-2 ${gallery.coverImageUrl ? "text-gray-200 drop-shadow" : "text-gray-500"}`}
            >
                ...
                <div className="text-center py-12">
                  <div className="flex flex-col items-center">
                    <div className="w-48 h-48 mb-6">
                      <WeddingImage
                        type="heart-balloon"
                        alt="Immagine decorativa di sposi"
                    ...
                    >
                      <div className="mb-8 border-b border-sage/20">
                        <TabsList className="relative mb-0 flex overflow-x-auto pb-0 bg-transparent gap-2 scrollbar-thin scrollbar-thumb-sage/20 scrollbar-track-transparent">
                          ...
                          >
                            Non assegnate (
                            {photos.filter((p) => !p.chapterId).length})
                          </TabsTrigger>
                            ...
                            <div className="text-center py-8">
                              <div className="flex flex-col items-center">
                                <div className="w-32 h-32 mb-4">
                                  <WeddingImage
                                    type="wedding-cake"
                                    alt="Immagine decorativa torta nuziale"
                      ...
                      {photos.map((photo, index) => (
                        ...
                        >
                          <img
                            ...
                            onLoad={(e) => {
                              // Imposta l'opacità a 1 quando l'immagine è caricata
                              (e.target as HTMLImageElement).classList.replace(
                                "opacity-0",
                                "opacity-100",
                              );
                            }}
            ...
            <div className="text-center md:text-left flex md:flex-row flex-col items-center gap-6">
              ...
              <div>
                <h3 className="text-xl font-playfair text-blue-gray font-medium mb-2">
                  Ti sono piaciute queste foto?
                </h3>
                <p className="text-gray-600 max-w-lg">
                  Segui il nostro profilo Instagram per vedere altri momenti
                  speciali come quelli del matrimonio e restare aggiornato sui
                  nostri servizi fotografici.
                </p>
              </div>
            </div>
