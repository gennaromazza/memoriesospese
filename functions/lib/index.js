...
exports.sendWelcomeEmail =
    exports.testEmailConfiguration =
    exports.sendGalleryPasswordV2 =
    exports.sendNewPhotosNotificationCall =
    exports.sendNewPhotosNotification =
    exports.getGalleryMetadata =
        void 0;
...
Object.defineProperty(exports, "getGalleryMetadata", {
    enumerable: true,
    get: function () {
        return metadata_1.getGalleryMetadata;
    },
});
 ...
 */
exports.sendNewPhotosNotification = functions.https.onRequest(
    async (req, res) => {
        corsHandler(req, res, async () => {
            try {
                ...
                const { sendGmailEmail, createNewPhotosEmailHTML } =
                    await Promise.resolve().then(() => require("./gmail"));
                // Crea HTML email
                ...
                functions.logger.info(
                    `New photos notification sent to ${recipients.length} recipients via Gmail API`,
                );
                res.status(200).json({
                    success: true,
                ...
                functions.logger.error(
                    "Error sending new photos notification:",
                    error,
                );
                res.status(500).json({
 ...
 */
exports.sendNewPhotosNotificationCall = functions.https.onCall(
    async (data, context) => {
        try {
            const {
                galleryName,
...
exports.sendGalleryPasswordV2 = functions
    ...
    .https.onRequest(async (req, res) => {
        ...
        try {
            ...
            functions.logger.info(
                `📧 Password request received for gallery ${galleryCode} (${galleryId})`,
            );
            ...
            if (!galleryPassword) {
                functions.logger.error(
                    `Gallery password not found: ${galleryId}`,
                );
                res.status(500).json({
                    error: {
                        code: "internal",
                        message: "Gallery password not configured",
                    },
                });
                return;
            }
            ...
            const hasSecurityQuestion =
                galleryData.requiresSecurityQuestion === true &&
                galleryData.securityQuestionType &&
                galleryData.securityAnswer;
            if (hasSecurityQuestion) {
                if (!securityAnswer) {
                    functions.logger.warn(
                        `Security answer required but not provided for gallery ${galleryId}`,
                    );
                    res.status(400).json({
                        error: {
                            code: "invalid-argument",
                ...
                if (providedAnswer !== correctAnswer) {
                    functions.logger.warn(
                        `Incorrect security answer for gallery ${galleryId}`,
                    );
                    res.status(403).json({
                        error: {
                ...
                functions.logger.info(
                    `Security question validated successfully for gallery ${galleryId}`,
                );
            }
            // Lazy import di gmail
            const { sendGmailEmail, createGalleryPasswordEmailHTML } =
                await Promise.resolve().then(() => require("./gmail"));
            // Crea HTML email
            ...
            functions.logger.info(
                `✅ Gallery password sent to ${recipientEmail} for gallery ${galleryName}`,
            );
            res.status(200).json({
                result: {
                    success: true,
                    message: "Gallery password sent successfully",
                    recipientEmail,
                },
            });
        } catch (error) {
            ...
            res.status(500).json({
                error: {
                    code: "internal",
                    message: "Failed to send gallery password email",
                },
            });
        }
    });
 ...
 */
exports.testEmailConfiguration = functions.https.onCall(
    async (data, context) => {
        try {
            const { testRecipient } = data;
            const recipient = testRecipient || "gennaro.mazzacane@gmail.com";
            // Lazy import di gmail (solo quando necessario)
            const { sendGmailEmail, createTestEmailHTML } =
                await Promise.resolve().then(() => require("./gmail"));
            // Crea HTML email
            ...
            functions.logger.info(
                `Test email sent to ${recipient} via Gmail API`,
            );
...
exports.sendWelcomeEmail = functions.https.onCall(async (data, context) => {
    try {
        ...
        // Lazy import di gmail (solo quando necessario)
        const { sendGmailEmail, createWelcomeEmailHTML } =
            await Promise.resolve().then(() => require("./gmail"));
        // Crea HTML email
        ...
        await sendGmailEmail(recipientEmail, subject, htmlContent);
        functions.logger.info(
            `Welcome email sent to ${recipientEmail} via Gmail API`,
        );
