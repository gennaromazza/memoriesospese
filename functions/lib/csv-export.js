...
exports.exportGalleryAccessCSV = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        if (!subscription || !subscription.active ||
            ...
            throw new functions.https.HttpsError('permission-denied', 'L\'esportazione CSV è disponibile solo per gli utenti Pro e Premium');
        }
        // Get gallery data
        ...
        if (accessSnapshot.empty) {
            throw new functions.https.HttpsError('not-found', 'Nessun accesso registrato per questa galleria');
        }
        ...
        throw new functions.https.HttpsError('internal', `Errore esportazione CSV: ${error.message}`);
    }
});
