...
export const generateGalleryZip = functions.https.onCall(async (data: any, context: any) => {
  if (!context.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
      'Devi essere autenticato per scaricare la galleria'
    );
  }
  ...
  try {
    ...
    if (!subscription || subscription.plan !== 'premium' || !subscription.active) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Il download ZIP è disponibile solo per gli utenti Premium'
      );
    }
    ...
    if (gallery?.userId !== userId) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Non hai i permessi per scaricare questa galleria'
      );
    }
    ...
    throw new functions.https.HttpsError(
      'internal',
      `Errore generazione ZIP: ${error.message}`
    );
  }
});