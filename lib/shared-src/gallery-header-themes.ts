/**
 * Definizioni condivise dei template visivi per l'intestazione della copertina
 * galleria. Web e desktop devono usare le stesse opzioni e lo stesso rendering.
 */
export interface GalleryHeaderTextStyle {
  color?: string;
  textShadow?: string;
  fontFamily?: string;
  fontWeight?: number;
...
export interface GalleryHeaderTheme {
  ...
  gradient: string;
  titleStyle: GalleryHeaderTextStyle;
  titleClass: string;
  metaStyle: GalleryHeaderTextStyle;
  lineColor: string;
...
export function getThemeById(id?: string | null): GalleryHeaderTheme {
  return GALLERY_HEADER_THEMES.find(theme => theme.id === id) ?? GALLERY_HEADER_THEMES[0];
}